...
interface USER {name:string,email:string,address:string,phone:string, id?:number}

async function createUser(user:USER){
    const users = await import(pathToFileURL(path.resolve('user.json')).href, { with: { type: "json" } }).then(m => m.default) as USER[];
